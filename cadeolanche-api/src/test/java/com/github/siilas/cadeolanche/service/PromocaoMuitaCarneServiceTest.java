...
public class PromocaoMuitaCarneServiceTest {

	private PromocaoMuitaCarneService service = new PromocaoMuitaCarneService();
	
	@Test
	public void deveValidarQuantidade() {
		ReciboResponse recibo = new ReciboResponse();
		recibo.setLanche(Lanche.from(Lanches.X_BURGER));
		recibo.getAdicionais().add(Ingrediente.from(Ingredientes.HAMBURGUER));
		
		Assert.assertEquals(Integer.valueOf(2), service.getQuantidadeCarne(recibo));
	}
